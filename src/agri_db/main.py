...
def ensure_schema(conn: psycopg.Connection) -> None:
    with conn.cursor() as cur:
        ...
        cur.execute("alter table usage_events enable row level security;")
        cur.execute("drop policy if exists usage_events_insert_anon on usage_events;")
        cur.execute(
            """
        ...
        cur.execute("drop view if exists usage_daily_metrics_jst;")
        cur.execute(
            """
        ...
        cur.execute("drop view if exists usage_monthly_metrics_jst;")
        cur.execute(
            """
        ...
        cur.execute("drop view if exists usage_daily_user_pv_jst;")
        cur.execute(
            """
        ...
        cur.execute("drop view if exists usage_monthly_user_pv_jst;")
        cur.execute(
            """
        ...
        cur.execute("drop view if exists usage_error_latest_7d_jst;")
        cur.execute(
            """
        ...
        cur.execute("grant select on table usage_daily_metrics_jst to anon, authenticated;")
        cur.execute("grant select on table usage_monthly_metrics_jst to anon, authenticated;")
